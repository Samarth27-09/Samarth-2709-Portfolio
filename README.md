...
1. Install dependencies (none required, but this ensures `node_modules` exists for future tooling):
   ```bash
   npm install
   ```
2. Start the local dev server with automatic MIME handling and 404 responses:
   ```bash
   npm run start
   ```
3. Visit [http://localhost:4173](http://localhost:4173) to view the site. Set the `PORT` environment variable to change the port if needed.
